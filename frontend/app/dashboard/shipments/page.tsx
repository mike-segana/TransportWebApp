        ...
        <div className="p-8">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-gray-500">Track your current and previous shipments</p>
                </div>
                <button 
                        ...
                        <tbody>
                            {shipments.map((shipment) => (
                                <tr key={shipment.id} className="border-b">
                                    ...
                                    <td className="p-3">{shipment.pickup_location}</td>
                                    <td className="p-3">{shipment.dropoff_location}</td>
                                    <td className="p-3">
                                        {shipment.status === "in_transit" ? (
                                            <span className="px-2 py-1 rounded bg-blue-100">{shipment.status}</span>
                                        ) : shipment.status === "completed" ? (
                                            <span className="px-2 py-1 rounded bg-green-100">{shipment.status}</span>
                                        ) : shipment.status === "assigned" ? (
                                            <span className="px-2 py-1 rounded bg-yellow-100">{shipment.status}</span>
                                        ) : (
                                            <span className="px-2 py-1 rounded bg-gray-100">{shipment.status}</span>
                                        )}
                                    </td>
                                </tr>
                            ))}
